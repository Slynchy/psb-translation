...
function main() {
	...
	if(isDirectory) console.log("Path is directory");

	if (process.argv[2] === 'decompile') {
		decompile(filename, isDirectory);
	} else if (process.argv[2] === 'recompile') {
		recompile(filename, isDirectory);
	} else {
		console.log('Invalid parameters');
	}
}

function recompile(filename, isDirectory) {
	if(isDirectory){
		console.log(`Recompiling files inside './${filename}'...`);
		const fileList = fs.readdirSync(filename);
		for(let i = 0; i < fileList.length; i++){
			const fileListFileName = `${filename}/${fileList[i]}`;
			recompile(fileListFileName);
		}
		return;
	}
...
function decompile(filename, isDirectory) {
	if(isDirectory){
		console.log(`Decompiling files inside './${filename}'...`);
		const fileList = fs.readdirSync(filename);
		for(let i = 0; i < fileList.length; i++){
			const fileListFileName = `${filename}/${fileList[i]}`;
			decompile(fileListFileName);
		}
		return;
	}

	console.log(`Decompiling ${filename}...`);
